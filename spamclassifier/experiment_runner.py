...
class ExperimentRunner:
    ...
    def run_experiment(
        ...
        compatible_scoring = self._get_compatible_scoring(
            pipeline, X_train, y_train, scoring
        )

        if param_grid:
            grid_search = GridSearchCV(
                pipeline, param_grid, cv=cv, scoring="f1_macro", n_jobs=4, verbose=3
            )
        ...
        default_cv_metrics = [
            "cv_test_accuracy",
            "cv_test_precision_macro",
            "cv_test_recall_macro",
            "cv_test_f1_macro",
            "cv_test_roc_auc_ovr",
        ]
