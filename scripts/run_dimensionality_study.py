...
def get_feature_count(X_train, y_train, preprocessor, vectorizer):
    ...
    temp_pipeline = Pipeline(
        [("preprocessor", preprocessor), ("vectorizer", vectorizer)]
    )
...
def create_adaptive_dimensionality_configs(n_features):
    ...
    for n_comp in [50, 100, min(200, n_features - 1)]:
        if n_comp < n_features:
            configs[f"SVD_{n_comp}"] = TruncatedSVD(
...
def main():
    ...
    for dim_name, dim_reducer in tqdm(
        adaptive_configs.items(), desc="Dimensionality Configs"
    ):
        try:
            if "PCA" in dim_name:
                pipeline = Pipeline(
            ...
            result = runner.run_experiment(
                ...
                pipeline,
                None,
                EXPERIMENT_CONFIG["cv"],
                EXPERIMENT_CONFIG["scoring"],
                "dimensionality_study",
                dim_name,
            )
            result["dimensionality_method"] = dim_name
            result["n_components"] = (
                dim_reducer.n_components
                if hasattr(dim_reducer, "n_components")
                else dim_reducer.k
            )
