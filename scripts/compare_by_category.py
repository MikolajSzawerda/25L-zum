        ...
        try:
            ...
            for category in unique_categories:
                ...
                if cat_count > 0:  # Make sure category exists in test set
                    ...
                    print(
                        f"  {category}: {cat_count} samples ({spam_count} spam, {ham_count} ham)"
                    )
                    ...
                    if len(np.unique(y_true_cat)) > 1:  # Both classes present
                        ...
                        precision, recall, f1_macro, _ = (
                            precision_recall_fscore_support(
                                y_true_cat, y_pred_cat, average="macro"
                            )
                        )
                        print(
                            f"    F1 (binary): {f1_cat:.3f}, F1 (macro): {f1_macro:.3f}"
                        )
...
def create_detailed_comparison_plot(results_df):
    ...
    pivot_df = category_results.pivot(
        index="category", columns="model", values="f1_score"
    )
    ...
    # Heatmap
    sns.heatmap(
        pivot_df, annot=True, fmt=".3f", cmap="RdYlBu_r", ax=ax1, vmin=0, vmax=1
    )
    ...
    category_results_grouped = (
        category_results.groupby("model")["f1_score"].agg(["mean", "std"]).reset_index()
    )

    bars = ax2.bar(
        category_results_grouped["model"],
        category_results_grouped["mean"],
        yerr=category_results_grouped["std"],
        capsize=5,
        alpha=0.7,
    )
...
def main():
    ...
    if not models:
        print(
            "No trained models found. Please run algorithm comparison experiment first."
        )
        return
    ...
    fig1.savefig(
        output_dir / "algorithm_category_comparison.png", dpi=300, bbox_inches="tight"
    )
    ...
    if fig2:
        fig2.savefig(
            output_dir / "algorithm_detailed_category_comparison.png",
            dpi=300,
            bbox_inches="tight",
        )
    ...
    if not category_results.empty:
        ...
        for category in category_results["category"].unique():
            ...
            print(
                f"  {category:<15}: {best_model['model']} (F1: {best_model['f1_score']:.3f})"
            )
